...
- [📂 Каталог документації](#-каталог-документації)
- [🛡️ Безпека та верифікація коду](#-безпека-та-верифікація-коду)
- [⚖️ Ліцензія](#-ліцензія)
...
> 3. **Безпека допоміжного демона відлагодження:** Допоміжний демон для відлагодження продакшену (`prod-mcp-gateway.js`) запускається локально на вашому комп'ютері та працює через мережу [Tailscale](https://tailscale.com/). Він **не має доступу** до файлової системи продакшн-сервера та не може прочитати ваш робочий файл `.env` чи секретні ключі. Відповідно, он не зможе передати їх ШІ-агенту.
...
![Бейджі безпеки](docs/assets/security-badges.png)

У верхній частині цього документа ви побачите два бейджі статусу:
- **Статус CodeQL (`CodeQL`)**: Працює на базі [вбудованого в GitHub Actions рушія CodeQL](https://codeql.github.com/). Зелений статус **`passing`** означає, що у вихідному коді C# не виявлено потенційних вразливостей (таких як SQL-ін'єкції, переповнення буфера чи обхід автентифікації).
- **Вразливості Snyk (`Snyk security`)**: Контролюється сервісом [Snyk](https://snyk.io/). Бейдж динамічно відображає статус відомих вразливостей та ліцензійних проблем у підключених NuGet та NPM пакетах. Успішний статус гарантує безпеку всіх зовнішніх залежностей.

> [!WARNING]
> **Статична перевірка НЕ гарантує безпеку виконання під час роботи програми!**
> 
> Навіть за успішного проходження всіх сканувань безпеки, цей проєкт **НЕ МОЖНА** запускати наосліп або надавати до нього доступ у ненадійних середовищах без додаткових заходів безпеки з таких причин:
> 1. **Динамічна поведінка ШІ-агентів:** Протокол контексту моделі (MCP) дозволяє ШІ-агентам динамічно виконувати консольні команди (наприклад, `npx`), робити запити до БД та читати/писати файли. Білі списки команд запобігають простим ін'єкціям, але логічні помилки в конфігурації все одно можливі.
> 2. **Ризики Prompt Injection:** Зловмисники можуть використовувати спеціально сформовані текстові повідомлення в Telegram, щоб змусити модель зловживати підключеними інструментами (наприклад, виконувати команди чи читати локальні файли).
> 3. **Ескалація привілеїв:** Запуск програми поза ізольованим контейнером (Docker) може надати ШІ-агенту доступ до локальної файлової системи хоста або запуск коду на вашому комп'ютері.
> 
> **Найбільш безпечний спосіб тестування та перевірки:**
> * **Контейнеризація:** Завжди запускайте програму в ізольованих Docker-контейнерах (використовуючи конфігурацію з [docker-compose.yml](docker-compose.yml) відповідно до [Інструкції з розгортання в Docker](docs/DOCKER.uk.md)), щоб обмежити доступ ШІ-агента до файлів та команд хост-системи.
> * **Ліміти API-ключів:** Використовуйте окремі API-ключі провайдерів (OpenAI, Gemini тощо) виключно для тестів та встановлюйте жорсткі ліміти бюджету у кабінетах провайдерів.
> * **Мінімальні права до БД:** Обмежуйте користувачів баз даних, які використовуються інструментами запитів MCP, правами "тільки для читання" (`SELECT`).
> * **Захищене з'єднання через [Tailscale](https://tailscale.com/):** Запускайте шлюз телеметрії (`prod-mcp-gateway.js`) тільки через ізольовані віртуальні мережі (наприклад, [Tailscale](https://tailscale.com/)) та ніколи не відкривайте його порти для публічного інтернету.
