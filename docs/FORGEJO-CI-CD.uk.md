# 🔄 CI/CD Розгортання через Forgejo Actions

🇺🇸 [English](FORGEJO-CI-CD.md) | 🇺🇦 [Українська](FORGEJO-CI-CD.uk.md)

Цей посібник охоплює **автоматизоване розгортання** GPTChatTelegramBot за допомогою Forgejo Actions на VPS.

---

## Огляд

Forgejo Actions сумісний з GitHub Actions workflows. Підрозділ CI/CD працює так само:

```mermaid
graph LR
    A[Push до prod] --> B[Forgejo Actions]
...
Додайте наступні secrets у налаштування Forgejo репозиторію (**Settings → Actions → Secrets**):
...
> [!IMPORTANT]
> **Ніколи** не фіксуйте secrets у системі контролю версій. Завжди використовуйте Secrets репозиторію для чутливих значень.
...
```yaml
name: Deploy via Forgejo Actions
...
```bash
ssh-keygen -t ed25519 -C "forgejo-actions-deploy"
```
...
### 3. Додайте Secrets до Forgejo

Перейдіть у **Settings → Actions → Secrets** та додайте всі secrets, перелічені вище.
...
### Перевірка Forgejo Actions

1. Перейдіть у ваш репозиторій на Forgejo
...
4. **Моніторьте** логи Forgejo Actions на підозрілу активність
