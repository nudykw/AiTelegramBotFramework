# 🔄 CI/CD Deployment via Forgejo Actions

🇺🇸 English | 🇺🇦 [Українська](FORGEJO-CI-CD.uk.md)

This guide covers **automated deployment** of GPTChatTelegramBot using Forgejo Actions to a VPS.

---

## Overview

Forgejo Actions is compatible with GitHub Actions workflows. The deployment pipeline works the same way:

```mermaid
graph LR
    A[Push to prod] --> B[Forgejo Actions]
    B --> C[Run Unit Tests]
    C --> D[SSH to VPS]
...
Add the following secrets to your Forgejo repository (**Settings → Actions → Secrets**):
...
> [!IMPORTANT]
> **Never** commit secrets to version control. Always use repository Secrets for sensitive values.
...
Go to **Settings → Actions → Secrets** and add all secrets listed above.
...
1. Go to your repository on Forgejo
2. Click **Actions** tab
...
4. **Monitor** Forgejo Actions logs for suspicious activity
